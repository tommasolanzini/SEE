...
# --- Configuration ---
# CHANGE THIS to your actual COM port (e.g., 'COM3' on Windows, '/dev/ttyACM0' on Linux/Mac)
COM_PORT = 'COM3' 
...
def run_single_test():
    try:
        ...
        with serial.Serial(COM_PORT, BAUD_RATE, timeout=5) as ser:
            time.sleep(1) # Give the MCU a moment to settle if opening the port resets it
            
            # 2. Generate a 590-byte dummy payload
            original_payload = os.urandom(PAYLOAD_SIZE)
            
            # 3. Send to MCU
            print("Sending 590 bytes to MCU...")
            ...
            if len(response) != TOTAL_RX_SIZE:
                print(f"Error: Expected {TOTAL_RX_SIZE} bytes, got {len(response)}.")
                return
